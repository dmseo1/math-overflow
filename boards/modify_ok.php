...
<?php
	...
	$sql_password = "select * from " . $boardname . " where no=" . $no;
	$result_password = $conn->query($sql_password);
	if(!$result_password || $result_password->num_rows == 0) //조작 접근 방지
	{
	...
	if($origin_member_no == -1) //비회원의 글 수정
	{
		if($password == $origin_password) //비회원 비밀번호 맞음
		{
			$sql = "update " . $boardname . " set title=\"" . $title . "\", content=\"" . $message . "\", writer=\"" . $writer . "\", imgpath1='" . $imgupload1 . "', imgpath2='" . $imgupload2 . "', imgpath3='" . $imgupload3 . "' where no=" . $no;
	...
	else //회원의 글 수정
	{
		...
		{
			if($origin_member_no == $_SESSION['user_member_no'])
			{
				$sql = "update " . $boardname . " set title=\"" . $title . "\", content=\"" . $message . "\", writer=\"" . $writer . "\", imgpath1='" . $imgupload1 . "', imgpath2='" . $imgupload2 . "', imgpath3='" . $imgupload3 . "' where no=" . $no;
