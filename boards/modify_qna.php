...
<div id="wrap">
  ...
  <div id="content">
	...
	{
		echo "<h2>" . $row_title["alias"] . " - 질문 수정하기</h2>";
	}
	...
	while($row_origin = $result_origin->fetch_assoc())
	{
		...
		$origin_points = $row_origin["points"];
		$origin_tags = $row_origin["tags"];
		$origin_member_no = $row_origin["member_no"];
		$origin_isfinished = $row_origin["isfinished"];
	...
	if($origin_isfinished == 1)
	{
		echo "<div style=\"width:50%;margin-left:100;margin-top:80;margin-bottom:500\">";
		echo "<table width=100%><tr><td colspan=2 style=\"padding-bottom:50px\"><center>채택/미채택 이후에는 질문을 수정할 수 없습니다.<br><br></center></td></tr>";
		echo "<tr><td width=50%><button style=\"width:100%\" type=\"button\" class=\"snip1535_list\" onclick=\"location.href='/index.html'\">홈으로</button></td>
				<td width=50%><button style=\"width:100%\" type=\"button\" class=\"snip1535_list\" onclick=\"history.back();\">뒤로
				</button></td>
	...
	echo "<form name=\"writeForm\" action=\"/boards/modify_qna_ok.php?boardname=" . $boardname . "&no=" . $no . "\" method=\"post\" enctype='multipart/form-data'>";
	...
	echo "<td width=90%>" . $origin_points . " &nbsp;&nbsp;- 포인트는 수정하실 수 없습니다.</td></tr>";
   ...
   echo "<td><div id=\"wmd-preview\" style=\"height:250;overflow:auto\" class=\"wmd-preview\"></div></td></tr>";
	...
	echo "<div style=\"width:90%;text-align:right;padding-top:10\"><button type='button' onclick=\"javascript:ChkBeforeWrite();\" class=\"snip1535_list\">작성 완료</button>&nbsp;&nbsp&nbsp;";
